	...
	internal sealed class RenderContext : IDimension
	{
		...
		internal void UpdateRenderContext(MK.Glow.ICameraData cameraData, RenderTextureFormat format, int depthBufferBits, RenderDimension dimension)
        {
			...
			else
			{
				#if UNITY_2018_3_OR_NEWER
				#if ENABLE_VR
				_descriptor.dimension = cameraData.GetStereoEnabled() ? UnityEngine.XR.XRSettings.eyeTextureDesc.dimension : UnityEngine.Rendering.TextureDimension.Tex2D;
				_descriptor.vrUsage = cameraData.GetStereoEnabled() ? UnityEngine.XR.XRSettings.eyeTextureDesc.vrUsage : VRTextureUsage.None;
				_descriptor.volumeDepth = cameraData.GetStereoEnabled() ? UnityEngine.XR.XRSettings.eyeTextureDesc.volumeDepth : 1;
				#else
